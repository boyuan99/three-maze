"""
EXPERIMENT_ID: hallway03
EXPERIMENT_NAME: Wide Hallway Experiment (No Fall Detection)
HARDWARE_MODE: standalone


Usage:
    experiment = Experiment("hallway03", config)
...
class Experiment:
    """
    Wide hallway experiment without fall detection.

    Based on hallway01_experiment.py with wider layout:
    - No fall detection (player stays at fixed height)
    - Wider hallway (40m, same as Hallway02)
    - Full walls (left, right, front, back)
    - Higher physics damping
    - Different encoder conversion factor

    Hardware Mode: STANDALONE
    - User directly initializes all hardware
    - Persistent DAQ task for water delivery
    - Complete transparency and control

    Key Methods:
    - process_serial_data(): Handle trackball input from serial port
    - process_position_update(): Sync with frontend physics engine
    """
    ...
    def __init__(self, experiment_id: str, config: Dict[str, Any], hardware_manager=None, event_callback=None):
        """
        Initialize wide hallway experiment.

        Args:
            experiment_id: Unique identifier (e.g., "hallway03")
        ...
        # Hallway dimensions (wider, same as hallway02)
        self.HALLWAY_LENGTH = 200.0
        self.HALLWAY_WIDTH = 40.0  # Wide hallway (same as Hallway02)
        self.WALL_HEIGHT = 10.0
        self.BLUE_SEGMENT_LENGTH = 30.0

        # Physics parameters
        self.PLAYER_RADIUS = 0.5
        self.PLAYER_HEIGHT = 2.0  # Fixed height (no gravity/fall detection)
        self.MAX_LINEAR_VELOCITY = 100.0
        self.DT = 1.0 / 20.0  # 20Hz

        # Conversion factor (matches JsSerialHallwayScene.vue)
    ...
    async def initialize(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.PLAYER_HEIGHT = config.get('playerHeight', self.PLAYER_HEIGHT)
        ...
        try:
            ...
            # Main data log (matches JsSerialHallwayScene.vue format)
        ...
        self.position = np.array([0.0, 0.0, self.PLAYER_HEIGHT, 0.0])
        ...
        logger.info(f"[{self.experiment_id}] Hallway width: {self.HALLWAY_WIDTH}m (wide version)")
        logger.info(f"[{self.experiment_id}] Using conversion factor: {self.ENCODER_TO_CM}")
        logger.info(f"[{self.experiment_id}] Player height fixed at: {self.PLAYER_HEIGHT}m")
    ...
    async def process_position_update(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - Check for trial end events

        NOTE: This experiment has NO fall detection (player stays at fixed height)
        ...
        Returns:
            Position dict with action and position data
        """
        if not self.is_active:
            return {
                'action': 'update',
        ...
        # Return position with action and current velocity
        if need_reset:
            self._reset_player()
            ...
            return {
                'action': 'set',
                'x': 0.0,
                'y': self.PLAYER_HEIGHT,  # Frontend Y = height (fixed at 2.0m)
    ...
    def _calculate_standardized_motion_realtime(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Convert displacement to velocity (matches frontend: raw * 0.0465 / DT)
            vx = np.clip(
            ...
            # World coordinate transformation (matches JsSerialHallwayScene.vue)
            ...
            return {
                'velocity': {
                    'x': world_vx,
                    'y': None,  # No vertical velocity (fixed height)
                    'z': world_vz
                },
    ...
    def _reset_player(self):
        ...
        Simple reset - no fall tracking (this experiment has no fall detection)
        """
        logger.info(f"[{self.experiment_id}] Resetting player to start position")

        # Reset position (fixed height at 2.0m)
        self.position = np.array([0.0, 0.0, self.PLAYER_HEIGHT, 0.0])
    ...
    def _log_data(self, water_delivered=False):
        ...
        if self.data_file and self.last_sensor_data:
            ...
            line = (
                ...
                f"wide70\n"                            # scene_name (wide hallway version)
            )
