"""
EXPERIMENT_ID: hallway01
EXPERIMENT_NAME: Simple Hallway Experiment (No Fall Detection)
HARDWARE_MODE: standalone


Usage:
    experiment = Experiment("hallway01", config)
...
class Experiment:
    """
    Simple hallway experiment without fall detection.

    Based on JsSerialHallwayScene.vue:
    - No fall detection (player stays at fixed height)
    - Narrower hallway (20m vs 40m)
    ...
    def __init__(self, experiment_id: str, config: Dict[str, Any], hardware_manager=None, event_callback=None):
        """
        Initialize simple hallway experiment.

        Args:
            experiment_id: Unique identifier (e.g., "hallway01")
        ...
        # Hallway dimensions (narrower than hallway02)
        self.HALLWAY_LENGTH = 200.0
        self.HALLWAY_WIDTH = 20.0  # Narrower hallway
